...
class library_flat_lensed_fixphi():
    """ a library which generates lensed CMB realizations from fixed phi map. """
    def __init__(self, pix, cl_unl, phi_fft, lib_dir, phase=None):
        """ initialize the library.
             pix     = a maps.pix object with the sky pixelization.
             cl_unl  = a spec.camb_clfile object containing the unlensed CMB temperature+polarization power spectra.
             phi_fft = fft of phi field used to lens maps.  a maps.rfft object
             lib_dir = directory to store the hash for the library.
             (optional) phase = phas.library object used to control the random number stream.
        """
        self.pix     = pix
        ...
        if self.phase == None:
            lmax = cl_unl.lmax
            self.phase = phas.library( 6*pix.nx*(pix.ny/2+1), lib_dir = self.lib_dir + "/phase" )
    ...
    def hashdict(self):
        phi_fft_hashdict = {'phi_fft':self.phi_fft.fft}
        return { 'pix'    : self.pix.hashdict(),
                 'cl_unl' : self.cl_unl.hashdict(),
                 'phase'  : self.phase.hashdict(),
                 'phi_fft': phi_fft_hashdict}
    ...
    def cache_sim(self, idx):
        ...
        tqu_unl = teb_unl.get_tqu()
        tqu_len = ql.lens.make_lensed_map_flat_sky( tqu_unl, self.phi_fft )
    ...
    def get_sim_phi(self, idx):
        return self.phi_fft.get_rmap()

    def get_sim_kappa(self, idx, fl=1.):
        return ( self.phi_fft  * ( 0.5 * np.arange(0., 10000.)**2 ) * fl ).get_rmap()
